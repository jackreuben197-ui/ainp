...
新增六组翻后纪律指标：`capped_pair_aggressive_actions` 统计口袋对子低于牌面或顶张以下一对仍主动做大底池；`four_straight_weak_continues` 统计四连张牌面上未成顺子仍继续投入；`paired_board_weak_aggression` 统计对子牌面形成的脆弱两对或主要由公共牌构成的牌力仍主动进攻；`three_flush_one_pair_aggression` 统计三同花牌面只有一对仍主动做大底池；`river_one_pair_reraises` 统计河牌只有一对仍对加注再次加注；`special_repeated_weak_aggression` 统计特殊风格同一手第二次及之后仍用弱牌主动进攻。六项门禁正常值均为零。对应的 `*_guards` 指标用于确认新版控池、弃牌和连续空枪阻断规则确实被调用。
