...
`FPCH_100_50` 是与正常强度体系分离的特殊模式。它 100% 主动入池，翻前按配置概率 Open Raise，否则 Call/Check；概率按手牌而不是按每次决策重复抽取，面对翻前加注不再 Re-Raise，每条翻后街最多主动加注一次。低 SPR 且仅使用公共牌形成牌型时会过牌控制底池，不再把底池尺度下注自动推成空气牌 All-in。该模式仍然永不主动 Fold，用于明确要求的特殊陪打场景，不能作为盈利型策略。Admin 独立显示其重复再加注、低胜率全下、动作、到达各轮次比例和 end_hand 净输赢。
