...
- 对子牌面形成的两对只有达到专用薄价值阈值才主动下注，低于阈值时即使是翻前进攻者也过牌控池；成牌主要来自公共牌或口袋对子仍低于牌面时，不会仅因对随机范围胜率较高而做价值加注。
- `aggressive_never_fold` 特殊风格仍保证不弃牌；当本手已经执行过激进行为，之后再遇到空气牌、脆弱一对/两对、公共牌主导牌力或四连张危险牌面时，有下注就跟注、可免费看牌就过牌，不再重复扩大弱牌底池。已改善为三条及以上的真实强牌不受弱牌特征残留影响。
...
同桌盈利保护在基础规则选出动作后、拟人化之前执行。它只处理达到配置资金暴露阈值的 Call/Bet/Raise/All-in，将“本次支付 BB 与剩余筹码占比”“同桌累计输赢与首次筹码占比”“实际 Call/激进行为比例”转换为有上限的附加胜率边际。边缘 Call 变为 Fold；边缘加注面对下注时优先降为仍满足边际的 Call，否则 Fold；无人下注时降为 Check。保护性 Fold 不允许被拟人化改回 Call。`aggressive_never_fold` 完全跳过该流程；已经配置专用 `large_pot_min_equity` 的风格（目前为经过百万手校准的 `FPCH_90_5`）全程以专用规则为准，不重复叠加通用保护。
