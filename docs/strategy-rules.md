...
- 激进行为不可用时安全降级；通常不会把普通动作擅自升级为 AllIn。唯一例外是开启 `collapse_near_allin`、请求明确将 AllIn 列为合法动作，且 Call/Bet/Raise 后剩余筹码命中调用方传入的操作前筹码百分比；调用方未传百分比时兼容使用 `near_allin_remaining_chips` 绝对尾筹阈值。该约束在所有人格和等级的最终动作出口统一执行，包括永不弃牌及合法动作降级产生的 Call。
