...
相关开关均位于 `engine.strategy`：`flop_air_call_margin`、`turn_air_call_margin`、`river_air_call_margin`、`repeated_air_call_penalty` 和 `reject_negative_ev_calls`。数值边际允许显式设为 0、布尔项允许设为 false，以保留旧策略；不改变任何 HTTP 请求或响应字段。
...
- 激进行为不可用时安全降级；通常不会把 Raise 擅自升级为 AllIn。唯一例外是开启 `collapse_near_allin`、Raise/Bet 后剩余筹码不超过 `near_allin_remaining_chips`，且请求明确将 AllIn 列为合法动作时，尾筹会归并为 AllIn。
