...
func TestIntentConflictIgnoresExplicitFoldGuard(t *testing.T) {
	item := entry{Action: "fold", RuleID: "PREFLOP_PROFILE_LARGE_CALL_FOLD"}
	if intentConflict(item) {
		t.Fatal("explicit fold guard was reported as an intent conflict")
	}
}
...
func TestAnalyzeCountsProfitControlApplications(t *testing.T) {
	path := filepath.Join(t.TempDir(), "ainp.jsonl")
	content := `{"msg":"strategy_decision","street":"flop","action":"fold","rule_id":"PROFIT_CONTROL_CALL_FOLD","profit_control_applied":true,"profit_control_total_margin":0.12}` + "\n" +
		`{"msg":"strategy_decision","street":"turn","action":"call","rule_id":"PROFIT_CONTROL_AGGRESSION_CALL","profit_control_applied":true,"profit_control_total_margin":0.08}` + "\n" +
		`{"msg":"strategy_decision","street":"river","action":"call","profit_control_total_margin":0.03}` + "\n"
	if err := os.WriteFile(path, []byte(content), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	if report.ProfitControlEvaluations != 3 || report.ProfitControlApplications != 2 || report.ProfitControlCallFolds != 1 || report.ProfitControlAggressionCuts != 1 {
		t.Fatalf("profit control evaluations=%d applications=%d call_folds=%d aggression_cuts=%d", report.ProfitControlEvaluations, report.ProfitControlApplications, report.ProfitControlCallFolds, report.ProfitControlAggressionCuts)
	}
}

func TestAnalyzeCountsFreeCallTotalNormalizations(t *testing.T) {
	path := filepath.Join(t.TempDir(), "ainp.jsonl")
	content := `{"msg":"game_state_normalized","normalization":"free_call_street_total"}` + "\n" +
		`{"msg":"game_state_normalized","normalization":"folded_hero_stale_action"}` + "\n" +
		`{"msg":"game_state_normalized","normalization":"another_compatibility"}` + "\n"
	if err := os.WriteFile(path, []byte(content), 0o600); err != nil {
		t.Fatal(err)
	}
	report, err := Analyze(path, Expectations{})
	...
	if report.FreeCallTotalNormalizations != 1 {
		t.Fatalf("normalizations=%d", report.FreeCallTotalNormalizations)
	}
	if report.FoldedHeroStreamNormalizations != 1 {
		t.Fatalf("folded hero normalizations=%d", report.FoldedHeroStreamNormalizations)
	}
}

func TestAnalyzeRejectsIncompleteShowdownOncePerTableHand(t *testing.T) {
	path := filepath.Join(t.TempDir(), "ainp.jsonl")
	content := `{"msg":"hand_result","table_id":"table","hand_id":"54","player_id":"a","reached_street":"flop","board_card_count":3,"showdown_players":4}` + "\n" +
		`{"msg":"hand_result","table_id":"table","hand_id":"54","player_id":"b","reached_street":"flop","board_card_count":3,"showdown_players":4}` + "\n" +
		`{"msg":"hand_result","table_id":"table","hand_id":"55","player_id":"a","reached_street":"river","board_card_count":5,"showdown_players":2}` + "\n"
	if err := os.WriteFile(path, []byte(content), 0o600); err != nil {
		t.Fatal(err)
	}
	report, err := Analyze(path, Expectations{})
	...
	if report.Passed || report.IncompleteShowdownHands != 1 || len(report.IncompleteShowdownExamples) != 1 {
		t.Fatalf("report=%+v", report)
	}
}

func TestAnalyzeAcceptsShowdownWhenAnotherPlayerStreamHasCompleteBoard(t *testing.T) {
	path := filepath.Join(t.TempDir(), "ainp.jsonl")
	content := `{"msg":"hand_result","table_id":"table","hand_id":"54","player_id":"folded","reached_street":"flop","board_card_count":3,"showdown_players":3}` + "\n" +
		`{"msg":"hand_result","table_id":"table","hand_id":"54","player_id":"active","reached_street":"river","board_card_count":5,"showdown_players":3}` + "\n"
	if err := os.WriteFile(path, []byte(content), 0o600); err != nil {
		t.Fatal(err)
	}
	report, err := Analyze(path, Expectations{})
	...
	if !report.Passed || report.IncompleteShowdownHands != 0 || len(report.IncompleteShowdownExamples) != 0 {
		t.Fatalf("report=%+v", report)
	}
}

func TestAnalyzeTracksAllInsSecondPairGuardAndSpecialReraises(t *testing.T) {
	path := filepath.Join(t.TempDir(), "ainp.jsonl")
	content := "" +
		`{"msg":"decision_event","decision_id":"a1","table_id":"t","hand_id":"h","player_id":"bot","outcome":"applied","advise":{"type":"allin","value":100}}` + "\n" +
		`{"msg":"decision_event","decision_id":"a2","table_id":"t","hand_id":"h","player_id":"bot","outcome":"applied","advise":{"type":"allin","value":20}}` + "\n" +
		`{"msg":"strategy_decision","decision_id":"s1","table_id":"t","hand_id":"h1","player_id":"normal","street":"flop","action":"raise","raises_faced":2,"one_pair_below_top_board":true}` + "\n" +
		`{"msg":"strategy_decision","decision_id":"s2","table_id":"t","hand_id":"h2","player_id":"normal","street":"flop","action":"fold","rule_id":"POSTFLOP_SECOND_PAIR_RERAISE_FOLD","raises_faced":2,"one_pair_below_top_board":true}` + "\n" +
		`{"msg":"strategy_decision","decision_id":"sp1","table_id":"t","hand_id":"h3","player_id":"special","ai_profile":"FPCH_100_50","audit_exempt":true,"street":"preflop","action":"raise","raises_faced":2,"equity":0.2}` + "\n" +
		`{"msg":"strategy_decision","decision_id":"sp2","table_id":"t","hand_id":"h3","player_id":"special","ai_profile":"FPCH_100_50","audit_exempt":true,"street":"flop","action":"allin","raises_faced":1,"equity":0.2}` + "\n"
	if err := os.WriteFile(path, []byte(content), 0o600); err != nil {
		t.Fatal(err)
	}
	report, err := Analyze(path, Expectations{})
	...
	if report.AdvisedAllIns != 2 || report.AdvisedAllInHands != 1 || report.SecondPairFeatureDecisions != 2 || report.SecondPairRepeatedRaises != 1 || report.SecondPairRepeatedRaiseFolds != 1 {
		t.Fatalf("report=%+v", report)
	}
	metric := report.SpecialProfileMetrics["FPCH_100_50"]
	if metric == nil || metric.FacingPreflopRaise != 1 || metric.PreflopReraises != 1 || metric.FacingPostflopRaise != 1 || metric.PostflopReraises != 1 || metric.LowEquityAllIns != 1 {
		t.Fatalf("special metric=%+v", metric)
	}
}

func TestAnalyzeTracksPostflopDisciplineAndHistoricalSpecialBarrels(t *testing.T) {
	path := filepath.Join(t.TempDir(), "ainp.jsonl")
	content := "" +
		`{"msg":"strategy_decision","decision_id":"capped-bet","table_id":"t","hand_id":"50","player_id":"normal","street":"river","action":"bet","hand_category":2,"hand_class":"made","one_pair_below_top_board":true}` + "\n" +
		`{"msg":"strategy_decision","decision_id":"capped-guard","table_id":"t","hand_id":"51","player_id":"normal","street":"river","action":"check","rule_id":"POSTFLOP_CAPPED_PAIR_POT_CONTROL","hand_category":2,"hand_class":"made","one_pair_below_top_board":true}` + "\n" +
		`{"msg":"strategy_decision","decision_id":"straight-call","table_id":"t","hand_id":"70","player_id":"normal","street":"turn","action":"call","hand_category":2,"four_to_straight_board":true,"active_opponents":2,"to_call":50,"pot":100}` + "\n" +
		`{"msg":"strategy_decision","decision_id":"straight-guard","table_id":"t","hand_id":"71","player_id":"normal","street":"turn","action":"fold","rule_id":"POSTFLOP_FOUR_STRAIGHT_FOLD","hand_category":2,"four_to_straight_board":true,"active_opponents":2,"to_call":50,"pot":100}` + "\n" +
		`{"msg":"strategy_decision","decision_id":"paired-bet","table_id":"t","hand_id":"90","player_id":"normal","street":"river","action":"allin","rule_id":"POSTFLOP_THIN_VALUE","hand_category":3,"paired_board_two_pair":true}` + "\n" +
		`{"msg":"strategy_decision","decision_id":"paired-thin-value","table_id":"t","hand_id":"90b","player_id":"normal","street":"turn","action":"bet","rule_id":"POSTFLOP_PAIRED_BOARD_THIN_VALUE","hand_category":3,"paired_board_two_pair":true}` + "\n" +
		`{"msg":"strategy_decision","decision_id":"paired-guard","table_id":"t","hand_id":"91","player_id":"normal","street":"river","action":"check","rule_id":"POSTFLOP_PAIRED_BOARD_POT_CONTROL","hand_category":3,"made_category_from_board":true}` + "\n" +
		`{"msg":"strategy_decision","decision_id":"flush-pair-raise","table_id":"t","hand_id":"92","player_id":"normal","street":"turn","action":"raise","rule_id":"POSTFLOP_VALUE_RAISE","hand_category":2,"hand_class":"made","three_to_flush_board":true}` + "\n" +
		`{"msg":"strategy_decision","decision_id":"flush-pair-guard","table_id":"t","hand_id":"93","player_id":"normal","street":"turn","action":"check","rule_id":"POSTFLOP_THREE_FLUSH_PAIR_POT_CONTROL","hand_category":2,"three_to_flush_board":true}` + "\n" +
		`{"msg":"strategy_decision","decision_id":"river-pair-reraise","table_id":"t","hand_id":"94","player_id":"normal","street":"river","raises_faced":1,"to_call":50,"pot":100,"action":"allin","rule_id":"POSTFLOP_ALLIN_VALUE","hand_category":2}` + "\n" +
		`{"msg":"strategy_decision","decision_id":"river-pair-guard","table_id":"t","hand_id":"95","player_id":"normal","street":"river","raises_faced":1,"to_call":50,"pot":100,"action":"fold","rule_id":"RIVER_ONE_PAIR_RERAISE_FOLD","hand_category":2}` + "\n" +
		`{"msg":"strategy_decision","decision_id":"special-flop","table_id":"t","hand_id":"116","player_id":"special","audit_exempt":true,"street":"flop","action":"bet"}` + "\n" +
		`{"msg":"strategy_decision","decision_id":"special-turn","table_id":"t","hand_id":"116","player_id":"special","audit_exempt":true,"street":"turn","action":"bet","hand_class":"air","hero_postflop_aggressive_actions":1}` + "\n" +
		`{"msg":"strategy_decision","decision_id":"special-river","table_id":"t","hand_id":"116","player_id":"special","audit_exempt":true,"street":"river","action":"check","rule_id":"SPECIAL_POSTFLOP_REPEAT_WEAK_BARREL_CHECK"}` + "\n"
	if err := os.WriteFile(path, []byte(content), 0o600); err != nil {
		t.Fatal(err)
	}
	report, err := Analyze(path, Expectations{MaxPreflopFoldRate: 1, MaxPreflopAggressionRate: 1, MaxHTTPErrorRate: 1})
	...
	if report.Passed || report.CappedPairAggressiveActions != 1 || report.CappedPairPotControlGuards != 1 ||
		report.FourStraightWeakContinues != 1 || report.FourStraightGuards != 1 ||
		report.PairedBoardWeakAggression != 1 || report.PairedBoardGuards != 1 ||
		report.ThreeFlushPairAggression != 1 || report.ThreeFlushPairGuards != 1 ||
		report.RiverOnePairReraises != 1 || report.RiverOnePairReraiseGuards != 1 ||
		report.SpecialRepeatedWeakAggression != 1 || report.SpecialRepeatedWeakBarrelGuards != 1 ||
		len(report.PostflopDisciplineExamples) != 6 {
		t.Fatalf("report=%+v", report)
	}
}

func TestAnalyzePostflopDisciplineExcludesIntentionalAndCallEquivalentActions(t *testing.T) {
	path := filepath.Join(t.TempDir(), "ainp.jsonl")
	content := "" +
		`{"msg":"strategy_decision","decision_id":"special","table_id":"t","hand_id":"1","player_id":"special","ai_profile":"FPCH_100_50","audit_exempt":true,"street":"turn","action":"raise","hand_category":2,"hand_class":"made","one_pair_below_top_board":true,"three_to_flush_board":true}` + "\n" +
		`{"msg":"strategy_decision","decision_id":"special-improved","table_id":"t","hand_id":"1b","player_id":"special","ai_profile":"FPCH_100_50","audit_exempt":true,"street":"river","action":"bet","hand_category":7,"hand_class":"made_strong","pocket_pair_under_board":true,"hero_postflop_aggressive_actions":1}` + "\n" +
		`{"msg":"strategy_decision","decision_id":"call-allin","table_id":"t","hand_id":"2","player_id":"normal","street":"turn","action":"allin","rule_id":"POSTFLOP_CALL","hand_category":2,"hand_class":"made","one_pair_below_top_board":true,"three_to_flush_board":true}` + "\n" +
		`{"msg":"strategy_decision","decision_id":"made-draw","table_id":"t","hand_id":"3","player_id":"normal","street":"turn","action":"raise","rule_id":"POSTFLOP_DRAW_RAISE","hand_category":2,"hand_class":"made_draw","one_pair_below_top_board":true,"three_to_flush_board":true}` + "\n" +
		`{"msg":"strategy_decision","decision_id":"priced-straight-call","table_id":"t","hand_id":"4","player_id":"normal","street":"turn","action":"call","hand_category":2,"hand_class":"made","four_to_straight_board":true,"active_opponents":1,"to_call":10,"pot":100}` + "\n" +
		`{"msg":"strategy_decision","decision_id":"paired-value","table_id":"t","hand_id":"5","player_id":"normal","street":"turn","action":"raise","rule_id":"POSTFLOP_VALUE_RAISE","hand_category":3,"hand_class":"made_strong","paired_board_two_pair":true,"equity":0.85}` + "\n"
	if err := os.WriteFile(path, []byte(content), 0o600); err != nil {
		t.Fatal(err)
	}
	report, err := Analyze(path, Expectations{})
	...
	if !report.Passed || report.CappedPairAggressiveActions != 0 || report.FourStraightWeakContinues != 0 ||
		report.PairedBoardWeakAggression != 0 || report.ThreeFlushPairAggression != 0 || report.RiverOnePairReraises != 0 ||
		len(report.PostflopDisciplineExamples) != 0 {
		t.Fatalf("report=%+v", report)
	}
}
...
func TestAnalyzeSeparatesRecoveredGameLogicErrors(t *testing.T) {
	path := filepath.Join(t.TempDir(), "ainp.jsonl")
	content := "" +
		`{"msg":"decision_event","decision_id":"bad","event_fingerprint":"bad-event","table_id":"t","hand_id":"h","player_id":"bot","seq_num":9,"cmd":"action","error_code":"Game logic error","error_message":"bet is invalid facing an existing bet","outcome":"rejected"}` + "\n" +
		`{"msg":"decision_event","decision_id":"replacement","table_id":"t","hand_id":"h","player_id":"bot","seq_num":9,"cmd":"action","outcome":"applied"}` + "\n"
	if err := os.WriteFile(path, []byte(content), 0o600); err != nil {
		t.Fatal(err)
	}
	report, err := Analyze(path, Expectations{})
	...
	if !report.Passed || report.GameLogicErrors != 0 || report.RecoveredGameLogicErrors != 1 || report.UniqueGameLogicErrors != 0 || len(report.GameLogicErrorExamples) != 0 {
		t.Fatalf("report=%+v", report)
	}
}

func TestAnalyzeDoesNotTreatDifferentCommandAsRecovery(t *testing.T) {
	path := filepath.Join(t.TempDir(), "ainp.jsonl")
	content := "" +
		`{"msg":"decision_event","decision_id":"bad-stage","event_fingerprint":"bad-stage-event","table_id":"t","hand_id":"h","player_id":"bot","seq_num":9,"cmd":"turn","error_code":"Game logic error","error_message":"turn cannot follow preflop","outcome":"rejected"}` + "\n" +
		`{"msg":"decision_event","decision_id":"later-action","table_id":"t","hand_id":"h","player_id":"bot","seq_num":9,"cmd":"action","outcome":"applied"}` + "\n"
	if err := os.WriteFile(path, []byte(content), 0o600); err != nil {
		t.Fatal(err)
	}
	report, err := Analyze(path, Expectations{})
	...
	if report.Passed || report.GameLogicErrors != 1 || report.RecoveredGameLogicErrors != 0 || report.UniqueGameLogicErrors != 1 {
		t.Fatalf("report=%+v", report)
	}
}
...
func TestAnalyzeTracksUnderpairCallsGuardsAndLegacyCandidates(t *testing.T) {
	...
	content := "" +
		...
		`{"msg":"strategy_decision","decision_id":"made-strong-call","table_id":"t","hand_id":"h1b","player_id":"bot","ai_profile":"FPCH_default","street":"turn","action":"call","hero_hand_class":"44","hand_class":"made_strong","pocket_pair_under_board":true,"equity":0.75,"pot_odds":0.20,"to_call":40}` + "\n" +
	...
	if report.UnderpairFeatureDecisions != 3 || report.UnderpairCalls != 1 || report.UnderpairCallHands != 1 ||
