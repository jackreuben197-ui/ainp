...
func TestAnalyzeRejectsStateErrorsAndActionDeviation(t *testing.T) {
	...
	content := "" +
		...
		`{"msg":"strategy_decision","decision_id":"d1","hand_id":"h1","player_id":"p1","ai_profile":"TAG_L2","profile_source":"profiles","personality_id":"tag","strategy_level":2,"target_vpip":0.3,"target_pfr":0.15,"street":"preflop","action":"raise","latency_us":10}` + "\n" +
	...
	if metric == nil || metric.StrategyLevel != 2 || metric.TargetVPIP != .3 || metric.TargetPFR != .15 || metric.PreflopHands != 2 || metric.VPIPRate != .5 || metric.PFRRate != .5 || metric.PreflopAggressionRate != .5 {
...
func TestLatePlayerStreamDeviationIsSeparatedFromLiveDeviation(t *testing.T) {
	...
	if !report.Passed || report.ActionTypeDeviations != 0 || report.ActionValueDeviations != 0 ||
		report.LatePlayerStreams != 1 || report.LateStreamTypeDeviations != 1 || report.LateStreamValueDeviations != 1 ||
