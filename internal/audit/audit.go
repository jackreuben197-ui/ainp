...
type Report struct {
	...
	FoldedHeroStreamNormalizations   int                              `json:"folded_hero_stream_normalizations"`
	...
	CappedPairAggressiveActions      int                              `json:"capped_pair_aggressive_actions"`
	CappedPairPotControlGuards       int                              `json:"capped_pair_pot_control_guards"`
	FourStraightWeakContinues        int                              `json:"four_straight_weak_continues"`
	FourStraightGuards               int                              `json:"four_straight_guards"`
	PairedBoardWeakAggression        int                              `json:"paired_board_weak_aggression"`
	PairedBoardGuards                int                              `json:"paired_board_guards"`
	ThreeFlushPairAggression         int                              `json:"three_flush_one_pair_aggression"`
	ThreeFlushPairGuards             int                              `json:"three_flush_one_pair_guards"`
	RiverOnePairReraises             int                              `json:"river_one_pair_reraises"`
	RiverOnePairReraiseGuards        int                              `json:"river_one_pair_reraise_guards"`
	SpecialRepeatedWeakAggression    int                              `json:"special_repeated_weak_aggression"`
	SpecialRepeatedWeakBarrelGuards  int                              `json:"special_repeated_weak_barrel_guards"`
	...
	ProfitControlEvaluations         int                              `json:"profit_control_evaluations"`
	ProfitControlApplications        int                              `json:"profit_control_applications"`
	ProfitControlCallFolds           int                              `json:"profit_control_call_folds"`
	ProfitControlAggressionCuts      int                              `json:"profit_control_aggression_cuts"`
	...
	PostflopDisciplineExamples       []DisciplineExample              `json:"postflop_discipline_examples,omitempty"`
...
type entry struct {
	...
	ActiveOpponents              int             `json:"active_opponents"`
	...
	PairedBoardTwoPair           *bool           `json:"paired_board_two_pair"`
	MadeCategoryFromBoard        *bool           `json:"made_category_from_board"`
	FourToStraightBoard          *bool           `json:"four_to_straight_board"`
	ThreeToFlushBoard            *bool           `json:"three_to_flush_board"`
	...
	HeroPostflopAggro            int             `json:"hero_postflop_aggressive_actions"`
	...
	ProfitControlApplied         bool            `json:"profit_control_applied"`
	ProfitControlTotalMargin     float64         `json:"profit_control_total_margin"`
	Profit                       float64         `json:"profit"`
	ReachedStreet                string          `json:"reached_street"`
...
func Analyze(path string, expected Expectations) (Report, error) {
	...
	showdownHands := make(map[string]showdownProgress)
	...
	for scanner.Scan() {
		...
		case "game_state_normalized":
			...
			case "free_call_street_total":
				r.FreeCallTotalNormalizations++
			case "folded_hero_stale_action":
				r.FoldedHeroStreamNormalizations++
			}
		...
		case "hand_result":
			...
			progress := showdownHands[tableHandKey]
			if progress.tableID == "" {
				progress.tableID, progress.handID, progress.playerID = item.TableID, item.HandID, item.PlayerID
			}
			if item.BoardCardCount > progress.boardCardCount {
				progress.boardCardCount = item.BoardCardCount
				progress.playerID = item.PlayerID
			}
			if item.ShowdownPlayers > progress.showdownPlayers {
				progress.showdownPlayers = item.ShowdownPlayers
			}
			showdownHands[tableHandKey] = progress
		...
		case "strategy_decision":
			if item.ProfitControlTotalMargin > 0 {
				r.ProfitControlEvaluations++
			}
			if item.ProfitControlApplied {
				r.ProfitControlApplications++
				if item.RuleID == "PROFIT_CONTROL_CALL_FOLD" {
					r.ProfitControlCallFolds++
				} else if strings.HasPrefix(item.RuleID, "PROFIT_CONTROL_AGGRESSION_") {
					r.ProfitControlAggressionCuts++
				}
			}
			featureCappedPair := (item.PocketPairUnderBoard != nil && *item.PocketPairUnderBoard) || (item.OnePairBelowTopBoard != nil && *item.OnePairBelowTopBoard)
			fourStraightWeak := item.FourToStraightBoard != nil && *item.FourToStraightBoard && item.HandCategory < poker.Straight
			if item.AuditExempt {
				if item.Street != "preflop" {
					weakMadeHand := item.HandClass == "air" ||
						(item.MadeCategoryFromBoard != nil && *item.MadeCategoryFromBoard) ||
						(item.PairedBoardTwoPair != nil && *item.PairedBoardTwoPair && item.HandCategory == poker.TwoPair) ||
						(item.HandClass == "made" && item.HandCategory == poker.OnePair && featureCappedPair) ||
						fourStraightWeak
					if isAggressiveDecision(item) && item.HeroPostflopAggro > 0 && weakMadeHand {
						r.SpecialRepeatedWeakAggression++
						appendDisciplineExample(&r, "special_repeated_weak_aggression", item)
					}
					if item.RuleID == "SPECIAL_POSTFLOP_REPEAT_WEAK_BARREL_CHECK" || item.RuleID == "SPECIAL_POSTFLOP_REPEAT_WEAK_BARREL_CALL" {
						r.SpecialRepeatedWeakBarrelGuards++
					}
				}
				key := eventHandKey(item)
			...
			cappedPair := item.HandCategory == poker.OnePair && item.HandClass == "made" && featureCappedPair
			if item.Street != "preflop" && cappedPair && isAggressiveDecision(item) {
				r.CappedPairAggressiveActions++
				appendDisciplineExample(&r, "capped_pair_aggressive_actions", item)
			}
			if item.RuleID == "POSTFLOP_CAPPED_PAIR_POT_CONTROL" || item.RuleID == "POSTFLOP_UNDERPAIR_PRESSURE_FOLD" || item.RuleID == "POSTFLOP_CAPPED_PAIR_FOLD" {
				r.CappedPairPotControlGuards++
			}
			fourStraightPressure := item.RaisesFaced > 0 || item.ActiveOpponents > 1 || item.ToCall >= .5*item.Pot
			if fourStraightWeak && fourStraightPressure && isContinueDecision(item) {
				r.FourStraightWeakContinues++
				appendDisciplineExample(&r, "four_straight_weak_continues", item)
			}
			if item.RuleID == "POSTFLOP_FOUR_STRAIGHT_POT_CONTROL" || item.RuleID == "POSTFLOP_FOUR_STRAIGHT_FOLD" {
				r.FourStraightGuards++
			}
			pairedBoardWeak := (item.MadeCategoryFromBoard != nil && *item.MadeCategoryFromBoard) || (item.PairedBoardTwoPair != nil && *item.PairedBoardTwoPair && item.RuleID == "POSTFLOP_THIN_VALUE")
			if pairedBoardWeak && isAggressiveDecision(item) {
				r.PairedBoardWeakAggression++
				appendDisciplineExample(&r, "paired_board_weak_aggression", item)
			}
			if item.RuleID == "POSTFLOP_PAIRED_BOARD_POT_CONTROL" || item.RuleID == "POSTFLOP_PAIRED_BOARD_RAISE_FOLD" {
				r.PairedBoardGuards++
			}
			threeFlushPair := item.ThreeToFlushBoard != nil && *item.ThreeToFlushBoard && item.HandCategory == poker.OnePair
			if threeFlushPair && item.HandClass == "made" && isAggressiveDecision(item) && aggressiveRule(item.RuleID) {
				r.ThreeFlushPairAggression++
				appendDisciplineExample(&r, "three_flush_one_pair_aggression", item)
			}
			if item.RuleID == "POSTFLOP_THREE_FLUSH_PAIR_POT_CONTROL" || item.RuleID == "POSTFLOP_THREE_FLUSH_PAIR_FOLD" {
				r.ThreeFlushPairGuards++
			}
			if item.Street == "river" && item.HandCategory == poker.OnePair && item.RaisesFaced > 0 && isAggressiveDecision(item) && aggressiveRule(item.RuleID) {
				r.RiverOnePairReraises++
				appendDisciplineExample(&r, "river_one_pair_reraises", item)
			}
			if item.RuleID == "RIVER_ONE_PAIR_RERAISE_FOLD" {
				r.RiverOnePairReraiseGuards++
			}
	...
	for _, progress := range showdownHands {
		if progress.showdownPlayers < 2 || progress.boardCardCount >= 5 {
			continue
		}
		...
		if len(r.IncompleteShowdownExamples) < 20 {
			r.IncompleteShowdownExamples = append(r.IncompleteShowdownExamples, ErrorExample{
				TableID: progress.tableID, HandID: progress.handID, PlayerID: progress.playerID,
				Message: fmt.Sprintf("showdown has %d players but only %d board cards reached AinP", progress.showdownPlayers, progress.boardCardCount),
			})
		}
	}
...
func appendDisciplineExample(r *Report, check string, item entry) {
	count := 0
	for _, existing := range r.PostflopDisciplineExamples {
	...
	r.PostflopDisciplineExamples = append(r.PostflopDisciplineExamples, DisciplineExample{Check: check, Example: exampleFromEntry(item)})
}
...
func exampleFromEntry(item entry) Example {
	...
	example := Example{
		...
		Action: item.Action, Amount: item.Amount, RuleID: item.RuleID, Equity: item.Equity, Stack: item.EffectiveStack,
...
func isAggressiveDecision(item entry) bool {
	return item.Action == "bet" || item.Action == "raise" || item.Action == "allin" && isActiveAllIn(item)
}

func isContinueDecision(item entry) bool {
	return isCallEquivalent(item) || isAggressiveDecision(item)
}

func isContinueAction(action string) bool {
	return action == "call" || isAggressiveAction(action)
}
...
func (r *Report) evaluate(e Expectations) {
	...
	check("capped_pair_aggressive_actions", r.CappedPairAggressiveActions == 0, "<=0", fmt.Sprint(r.CappedPairAggressiveActions))
	check("four_straight_weak_continues", r.FourStraightWeakContinues == 0, "<=0", fmt.Sprint(r.FourStraightWeakContinues))
	check("paired_board_weak_aggression", r.PairedBoardWeakAggression == 0, "<=0", fmt.Sprint(r.PairedBoardWeakAggression))
	check("three_flush_one_pair_aggression", r.ThreeFlushPairAggression == 0, "<=0", fmt.Sprint(r.ThreeFlushPairAggression))
	check("river_one_pair_reraises", r.RiverOnePairReraises == 0, "<=0", fmt.Sprint(r.RiverOnePairReraises))
	check("special_repeated_weak_aggression", r.SpecialRepeatedWeakAggression == 0, "<=0", fmt.Sprint(r.SpecialRepeatedWeakAggression))
