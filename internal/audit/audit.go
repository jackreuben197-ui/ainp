...
type LateStreamExample struct {
	...
	HandID          string `json:"hand_id"`
	PlayerID        string `json:"player_id"`
	StartAfterEndMS int64  `json:"start_after_end_ms"`
}
...
func Analyze(path string, expected Expectations) (Report, error) {
	...
	latePlayerStreams := make(map[string]int64)
	...
	for scanner.Scan() {
		...
		case "decision_event":
			...
			if item.Cmd == "start_hand_extended" && item.Outcome == "applied" && !item.Time.IsZero() {
				if endedAt, ok := tableHandEndedAt[tableKey]; ok {
					...
					if delta >= lateStreamThreshold {
						latePlayerStreams[key] = delta.Milliseconds()
						r.LatePlayerStreams++
						if len(r.LateStreamExamples) < 20 {
							r.LateStreamExamples = append(r.LateStreamExamples, LateStreamExample{
								DecisionID: item.DecisionID, TableID: item.TableID, HandID: item.HandID,
								PlayerID: item.PlayerID, StartAfterEndMS: delta.Milliseconds(),
							})
						}
					}
				}
			}
			lateStreamDelayMS := latePlayerStreams[key]
			...
			if item.ErrorCode == "Game logic error" {
				r.GameLogicErrors++
				key := item.EventFingerprint + "|" + item.ErrorMessage
				if item.EventFingerprint == "" {
					key = item.DecisionID + "|" + item.ErrorMessage
				}
				example := logicErrorExamples[key]
				if example == nil {
					...
					logicErrorExamples[key] = example
				}
				example.Occurrences++
			}
			...
			if item.Deviation != nil && specialProfile == "" {
				...
				example.StartAfterEndMS = lateStreamDelayMS
		...
		case "strategy_decision":
			...
			if item.Action == "allin" && item.Equity < lowEquityAllIn && !strings.Contains(item.RuleID, "CALL") {
				r.LowEquityAllIns++
			}
...
func exampleFromEntry(item entry) Example {
	...
	return Example{
		...
		ToCall: item.ToCall, Pot: item.Pot, PotOdds: item.PotOdds, CallEV: item.CallEV,
	}
}
