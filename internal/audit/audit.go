...
func Analyze(path string, expected Expectations) (Report, error) {
	...
	specialPostflopAggression := make(map[string]int)
	...
	for scanner.Scan() {
		...
		case "strategy_decision":
			cappedPair := (item.PocketPairUnderBoard != nil && *item.PocketPairUnderBoard) || (item.OnePairBelowTopBoard != nil && *item.OnePairBelowTopBoard)
			if item.Street != "preflop" && cappedPair && isAggressiveAction(item.Action) {
			...
			if fourStraightWeak && isContinueAction(item.Action) {
			...
			pairedBoardWeak := (item.PairedBoardTwoPair != nil && *item.PairedBoardTwoPair) || (item.MadeCategoryFromBoard != nil && *item.MadeCategoryFromBoard)
			if pairedBoardWeak && isAggressiveAction(item.Action) && item.RuleID != "POSTFLOP_PAIRED_BOARD_THIN_VALUE" {
			...
			if threeFlushPair && isAggressiveAction(item.Action) && aggressiveRule(item.RuleID) {
			...
			if item.Street == "river" && item.HandCategory == poker.OnePair && item.RaisesFaced > 0 && isAggressiveAction(item.Action) && aggressiveRule(item.RuleID) {
				r.RiverOnePairReraises++
			}
			if item.RuleID == "RIVER_ONE_PAIR_RERAISE_FOLD" {
				r.RiverOnePairReraiseGuards++
			}
			if item.AuditExempt && item.Street != "preflop" {
				key := eventHandKey(item)
				if isAggressiveAction(item.Action) {
					weakMadeHand := item.HandClass == "air" || pairedBoardWeak || cappedPair || fourStraightWeak
					if specialPostflopAggression[key] > 0 && weakMadeHand {
						r.SpecialRepeatedWeakAggression++
					}
					specialPostflopAggression[key]++
				}
				if item.RuleID == "SPECIAL_POSTFLOP_REPEAT_WEAK_BARREL_CHECK" {
