...
func TestEmbeddedDashboardIsAvailable(t *testing.T) {
	...
	html := string(page)
	for _, want := range []string{`id="special-losses"`, `id="decision-modal"`, `id="special-loss-decisions"`, `id="river-pair-examples"`, `data-help="river_missed_draw_calls"`, `id="preflop-large-call-examples"`, `data-help="preflop_large_call_guard"`, `preflop_large_call_misses`, `legacy_reraised_preflop_calls`, `id="underpair-examples"`, `data-help="underpair_guard"`, `legacy_underpair_call_candidates`, `underpair_guard_folds`, `river_repeated_missed_draw_board_pair_calls`, `missed_straight_draw`, `迟到补发玩家事件流`, `id="late-stream-summary"`, `late_streams_after_table_progress`, `after_table_progress`, `table_seq_at_start`, `data-special-profile`, `class="stack"`, `overflow-x:hidden`} {
