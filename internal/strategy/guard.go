	...
	if desired == Fold && g.req.NeverFold {
		for _, action := range []Action{Call, Check, AllIn} {
			if legal, ok := g.actions[action]; ok {
				amount := g.amount(legal, g.req.ToCall)
				if g.shouldCollapseNearAllIn(action, amount) {
...
// shouldCollapseNearAllIn prevents a call, raise, or bet from leaving
...
func (g *actionGuard) shouldCollapseNearAllIn(action Action, amount float64) bool {
	if !g.req.CollapseNearAllIn || (action != Call && action != Bet && action != Raise) {
		return false
	}
