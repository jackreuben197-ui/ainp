...
func buildFeatures(req Request) (Features, error) {
	...
	if !omaha {
		features.PairedBoardTwoPair = rank.Category == poker.TwoPair && boardHasPair(req.Board)
		features.FourToStraightBoard = boardFourToStraight(req.Board, minRank)
		features.ThreeToFlushBoard = boardThreeToFlush(req.Board)
		if len(req.Board) == 5 {
			boardRank, boardErr := poker.EvaluateWithRules(req.Board, rules)
			if boardErr != nil {
				return Features{}, boardErr
			}
			features.MadeCategoryFromBoard = boardRank.Category == rank.Category && rank.Category >= poker.OnePair && rank.Category <= poker.TwoPair
		}
	}
	...
	switch {
	case rank.Category >= poker.TwoPair && !features.PairedBoardTwoPair && !features.MadeCategoryFromBoard:
...
func boardThreeToFlush(board []poker.Card) bool {
	var suits [4]int
...
func boardFourToStraight(board []poker.Card, minRank int) bool {
	if len(board) < 4 {
		return false
	}
	ranks := make(map[int]bool, len(board)+1)
	for _, card := range board {
		ranks[card.Rank()] = true
	}
	// Ace can be the low end of the wheel in standard hold'em.
	if minRank == 2 && ranks[14] {
		ranks[1] = true
	}
	for high := 5; high <= 14; high++ {
		if high-4 < minRank && high != 5 {
			continue
		}
		count := 0
