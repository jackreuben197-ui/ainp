...
func TestStrategyDecisionLogIsStructuredAndDoesNotContainCards(t *testing.T) {
	...
	text := logs.String()
	for _, expected := range []string{`"msg":"strategy_decision"`, `"decision_id":"decision-1"`, `"rule_id":"PREFLOP_OPEN"`, `"personality_id":"balanced"`, `"river_card_features_available":false`, `"pair_from_board_only":false`, `"missed_flush_draw":false`, `"missed_straight_draw":false`} {
...
func TestRiverFeaturesIdentifyMissedStraightDrawWithBoardOnlyPair(t *testing.T) {
	...
	if features.Category != poker.OnePair || features.Class != ClassMade || !features.RiverCardFeaturesAvailable || !features.PairFromBoardOnly || features.MissedFlushDraw || !features.MissedStraightDraw {
...
func TestRiverFeaturesIdentifyMissedFlushDrawWithBoardOnlyPair(t *testing.T) {
	...
	if features.Category != poker.OnePair || features.Class != ClassMade || !features.RiverCardFeaturesAvailable || !features.PairFromBoardOnly || !features.MissedFlushDraw {
		t.Fatalf("features=%+v", features)
	}
...
func TestWeakDrawAndBoardPairFoldsAreProtectedFromHumanization(t *testing.T) {
	for _, ruleID := range []string{"TURN_WEAK_DRAW_FOLD", "POSTFLOP_UNDERPAIR_FOLD", "RIVER_BOARD_PAIR_BLUFF_CATCH_FOLD"} {
