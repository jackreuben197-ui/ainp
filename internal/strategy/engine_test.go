...
func TestSpecialProfileCallsInsteadOfRepeatingWeakAggressionFacingBet(t *testing.T) {
	req := Request{
		PlayerID: "special", TableID: "table", HandID: "hand", AIProfile: "FPCH_100_50",
		Street: River, Pot: 100, ToCall: 30, Stack: 200, PostflopAggressionChance: 1,
		HeroPostflopAggro: 1,
	}
	features := Features{Category: poker.OnePair, Class: ClassMade, OnePairBelowTopBoard: true}
	action, amount, rule, tags, aggressive := chooseAggressiveNeverFold(req, Decision{SPR: 2, Features: features})
	if action != Call || amount != req.ToCall || rule != "SPECIAL_POSTFLOP_REPEAT_WEAK_BARREL_CALL" || aggressive || !slices.Contains(tags, "always_continue") {
		t.Fatalf("action=%s amount=%v rule=%s tags=%v aggressive=%v", action, amount, rule, tags, aggressive)
	}
}

func TestSpecialProfileDoesNotTreatImprovedStrongHandAsUnderpair(t *testing.T) {
	req := Request{
		PlayerID: "special", TableID: "table", HandID: "strong", AIProfile: "FPCH_100_50",
		Street: River, Pot: 100, Stack: 200, PostflopAggressionChance: 1,
		HeroPostflopAggro: 1,
	}
	features := Features{Category: poker.FullHouse, Class: ClassMadeStrong, PocketPairUnderBoard: true}
	action, _, rule, _, _ := chooseAggressiveNeverFold(req, Decision{SPR: 2, Features: features})
	if action != Bet || rule != "SPECIAL_POSTFLOP_BET" {
		t.Fatalf("action=%s rule=%s", action, rule)
	}
}

func TestPairedBoardTwoPairBelowThinValueThresholdChecks(t *testing.T) {
	profile, _ := personality.Resolve("balanced")
	req := Request{Street: Turn, Pot: 100, Stack: 200, EffectiveStack: 200, WasPreflopAggressor: true}
	decision := Decision{
		Equity:   equity.Result{Equity: .60},
		Features: Features{Category: poker.TwoPair, Class: ClassMade, PairedBoardTwoPair: true},
	}
	action, amount, rule, _, aggressive := choosePostflopAdjusted(req, decision, 5, profile, OpponentRead{BluffMultiplier: 1})
	if action != Check || amount != 0 || rule != "POSTFLOP_PAIRED_BOARD_POT_CONTROL" || aggressive {
		t.Fatalf("action=%s amount=%v rule=%s aggressive=%v", action, amount, rule, aggressive)
	}
}

func TestBoardDependentAndUnderpairHandsDoNotValueRaise(t *testing.T) {
	profile, _ := personality.Resolve("balanced")
	req := Request{
		Street: Turn, Pot: 100, ToCall: 10, Stack: 200, EffectiveStack: 200,
		LegalActions: []LegalAction{{Action: Fold}, {Action: Call, Min: 10, Max: 10}, {Action: Raise, Min: 30, Max: 200}},
	}
	for _, test := range []struct {
		name     string
		features Features
	}{
		{name: "underpair", features: Features{Category: poker.OnePair, Class: ClassMade, PocketPairUnderBoard: true}},
		{name: "made from board", features: Features{Category: poker.TwoPair, Class: ClassMadeStrong, MadeCategoryFromBoard: true}},
	} {
		t.Run(test.name, func(t *testing.T) {
			decision := Decision{Equity: equity.Result{Equity: .90}, PotOdds: .09, CallEV: 80, SPR: 2, Features: test.features}
			action, amount, rule, _, aggressive := choosePostflopAdjusted(req, decision, 5, profile, OpponentRead{BluffMultiplier: 1})
			if action != Call || amount != req.ToCall || rule != "POSTFLOP_CALL" || aggressive {
				t.Fatalf("action=%s amount=%v rule=%s aggressive=%v", action, amount, rule, aggressive)
			}
		})
	}
