...
func chooseAggressiveNeverFold(req Request, decision Decision) (Action, float64, string, []string, bool) {
	...
	if req.ToCall == 0 && req.HeroPostflopAggro > 0 && (decision.Features.Class == ClassAir || decision.Features.PairedBoardTwoPair || decision.Features.MadeCategoryFromBoard) {
...
func choosePostflopAdjusted(req Request, decision Decision, level int, profile personality.Profile, read OpponentRead) (Action, float64, string, []string, bool) {
	...
	if canRaiseForValue && !riverOnePair && !threeFlushPairBluffCatch && !decision.Features.OnePairBelowTopBoard && (eq >= .74+profile.AggressionDelta || class == ClassMadeStrong && eq >= .50 && eq-decision.PotOdds >= .18+profile.AggressionDelta) {
		if decision.SPR <= 1.2 {
