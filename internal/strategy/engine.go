...
func (e *Engine) Decide(ctx context.Context, req Request) (Decision, error) {
	...
	protectedFold := desired == Fold && (ruleID == "POSTFLOP_NEGATIVE_EV_FOLD" || ruleID == "POSTFLOP_AIR_MARGIN_FOLD")
...
func chooseRule(req Request, decision Decision, profile personality.Profile) (Action, float64, string, []string, bool) {
	...
	if req.BehaviorMode == "aggressive_never_fold" {
		return chooseAggressiveNeverFold(req)
	}
	if req.Street == Preflop {
...
func chooseAggressiveNeverFold(req Request) (Action, float64, string, []string, bool) {
	...
	if req.Street == Preflop {
		if probabilityRoll(stableSpecialBehaviorSeed(req), 101) < req.PreflopRaiseProbability {
			amount := req.ToCall + math.Max(3*req.BigBlind, req.Pot)
	...
	if probabilityRoll(stableSpecialBehaviorSeed(req), 211+int64(len(req.Board))) < req.PostflopAggressionChance {
		if req.ToCall > 0 {
			return Raise, req.ToCall + math.Max(req.Pot, 2*req.ToCall), "SPECIAL_POSTFLOP_RAISE", append(tags, "probability_aggression"), true
		}
		return Bet, req.Pot, "SPECIAL_POSTFLOP_BET", append(tags, "probability_aggression"), true
	}
	if req.ToCall > 0 {
...
func chooseLegacyPreflop(req Request, eq, potOdds float64, level int, profile personality.Profile, read OpponentRead) (Action, float64, string, []string, bool) {
	...
	if req.ToCall == 0 {
		if eq >= threshold {
			return Raise, math.Max(2.5*req.BigBlind, .6*req.Pot), "PREFLOP_OPEN", []string{"position_range", "value_raise"}, true
		}
	...
	if req.RaisesFaced == 0 {
		if eq >= threshold {
			return Raise, req.ToCall + math.Max(2.5*req.BigBlind, .6*req.Pot), "PREFLOP_OPEN", []string{"position_range", "open_raise"}, true
		}
		if eq >= callThreshold {
	...
	if eq >= reraiseThreshold {
		...
		return Raise, req.ToCall + math.Max(2*req.ToCall, .75*req.Pot), "PREFLOP_RERAISE", []string{"premium", "value_reraise"}, true
	}
...
func chooseRateControlledPreflop(req Request, percentile float64) (Action, float64, string, []string, bool) {
	...
	if req.ToCall == 0 {
		if percentile <= req.TargetPFR {
			return Raise, math.Max(2.5*req.BigBlind, .6*req.Pot), "PREFLOP_PROFILE_RAISE", append(tags, "pfr_range"), true
		}
		return Check, 0, "PREFLOP_PROFILE_CHECK", append(tags, "no_vpip_opportunity"), false
	}
	if percentile <= req.TargetPFR {
		if req.RaisesFaced == 0 {
			return Raise, req.ToCall + math.Max(2.5*req.BigBlind, .6*req.Pot), "PREFLOP_PROFILE_RAISE", append(tags, "pfr_range"), true
		}
		if req.EffectiveStack <= req.Pot+2*req.ToCall {
			return AllIn, req.Stack, "PREFLOP_PROFILE_ALLIN", append(tags, "pfr_range", "low_spr"), true
		}
		return Raise, req.ToCall + math.Max(2*req.ToCall, .75*req.Pot), "PREFLOP_PROFILE_RERAISE", append(tags, "pfr_range"), true
	}
...
func choosePostflopAdjusted(req Request, decision Decision, level int, profile personality.Profile, read OpponentRead) (Action, float64, string, []string, bool) {
	...
	if req.ToCall == 0 {
		...
		case eq >= .70+profile.ValueThresholdDelta+read.ValueThreshold || class == ClassMadeStrong && eq >= .62+profile.ValueThresholdDelta+read.ValueThreshold:
			return Bet, .75 * betMultiplier * req.Pot, "POSTFLOP_VALUE_BET", []string{"strong_made", "value"}, true
		case (class == ClassDraw || class == ClassMadeDraw) && eq >= .40+profile.AggressionDelta:
			return Bet, .5 * betMultiplier * req.Pot, "POSTFLOP_SEMIBLUFF", []string{"draw", "semi_bluff"}, true
		case class == ClassMade && eq >= .52 && req.WasPreflopAggressor:
			return Bet, .5 * betMultiplier * req.Pot, "POSTFLOP_THIN_VALUE", []string{"made_hand", "initiative"}, true
		case class == ClassAir && req.Street == Flop && req.WasPreflopAggressor && bluffRollAdjusted(req.Seed, level, req.Position, profile.BluffMultiplier*read.BluffMultiplier):
			return Bet, .34 * betMultiplier * req.Pot, "FLOP_CBET_BLUFF", []string{"air", "cbet", "controlled_bluff"}, true
		default:
	...
	if canRaiseForValue && (class == ClassDraw || class == ClassMadeDraw) && eq >= .68+profile.AggressionDelta {
		...
		return Raise, req.ToCall + math.Max(req.Pot, 2*req.ToCall), "POSTFLOP_DRAW_RAISE", []string{"draw", "semi_bluff"}, true
	}
	if canRaiseForValue && (eq >= .74+profile.AggressionDelta || class == ClassMadeStrong && eq >= .50 && eq-decision.PotOdds >= .18+profile.AggressionDelta) {
		if decision.SPR <= 1.2 {
			return AllIn, req.Stack, "POSTFLOP_ALLIN_VALUE", []string{"strong_made", "low_spr"}, true
		}
		return Raise, req.ToCall + math.Max(req.Pot, 2*req.ToCall), "POSTFLOP_VALUE_RAISE", []string{"strong_made", "value_raise"}, true
	}
