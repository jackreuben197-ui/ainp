...
func (e *Engine) Decide(ctx context.Context, req Request) (Decision, error) {
	...
	desired, amount, ruleID, tags, aggressive, decision.ProfitControl = applyProfitControl(req, decision, desired, amount, ruleID, tags, aggressive)
	humanSeed := req.Seed
	if humanSeed == 0 {
...
func isProtectedPostflopFold(ruleID string) bool {
	...
	case "POSTFLOP_NEGATIVE_EV_FOLD",
		"PROFIT_CONTROL_CALL_FOLD",
		"PROFIT_CONTROL_AGGRESSION_FOLD",
		"POSTFLOP_AIR_MARGIN_FOLD",
		"TURN_WEAK_DRAW_FOLD",
		"POSTFLOP_UNDERPAIR_FOLD",
		"POSTFLOP_UNDERPAIR_PRESSURE_FOLD",
		"POSTFLOP_CAPPED_PAIR_FOLD",
		"POSTFLOP_FOUR_STRAIGHT_FOLD",
		"POSTFLOP_PAIRED_BOARD_RAISE_FOLD",
		"POSTFLOP_THREE_FLUSH_PAIR_FOLD",
		"RIVER_ONE_PAIR_RERAISE_FOLD",
...
func (e *Engine) logDecision(req Request, decision Decision, elapsed time.Duration) {
	...
	e.logger.Info("strategy_decision",
		...
		"paired_board_two_pair", decision.Features.PairedBoardTwoPair,
		"made_category_from_board", decision.Features.MadeCategoryFromBoard,
		"four_to_straight_board", decision.Features.FourToStraightBoard,
		"three_to_flush_board", decision.Features.ThreeToFlushBoard,
		...
		"spr", decision.SPR,
		"table_initial_stack", req.TablePerformance.InitialStack,
		"table_net_profit", req.TablePerformance.NetProfit,
		"table_profit_hands", req.TablePerformance.Hands,
		"table_action_count", req.TablePerformance.Actions,
		"table_call_rate", decision.ProfitControl.CallRate,
		"table_aggression_rate", decision.ProfitControl.AggressionRate,
		"profit_control_applied", decision.ProfitControl.Applied,
		"profit_control_action_bb", decision.ProfitControl.ActionBB,
		"profit_control_action_stack_ratio", decision.ProfitControl.ActionStackRatio,
		"profit_control_profit_bb", decision.ProfitControl.ProfitBB,
		"profit_control_profit_stack_ratio", decision.ProfitControl.ProfitStackRatio,
		"profit_control_exposure_margin", decision.ProfitControl.ExposureMargin,
		"profit_control_performance_margin", decision.ProfitControl.PerformanceMargin,
		"profit_control_action_mix_margin", decision.ProfitControl.ActionMixMargin,
		"profit_control_total_margin", decision.ProfitControl.TotalMargin,
		"hero_postflop_calls", req.HeroPostflopCalls,
		"hero_postflop_aggressive_actions", req.HeroPostflopAggro,
...
func chooseAggressiveNeverFold(req Request, decision Decision) (Action, float64, string, []string, bool) {
	...
	if req.HeroPostflopAggro > 0 && isWeakSpecialBarrel(decision.Features) {
		if req.ToCall > 0 {
			return Call, req.ToCall, "SPECIAL_POSTFLOP_REPEAT_WEAK_BARREL_CALL", append(tags, "repeat_weak_barrel", "pot_control", "always_continue"), false
		}
		return Check, 0, "SPECIAL_POSTFLOP_REPEAT_WEAK_BARREL_CHECK", append(tags, "repeat_weak_barrel", "pot_control"), false
	}
...
func isWeakSpecialBarrel(features Features) bool {
	return features.Class == ClassAir ||
		features.MadeCategoryFromBoard ||
		(features.PairedBoardTwoPair && features.Category == poker.TwoPair) ||
		(features.Class == ClassMade && features.Category == poker.OnePair && (features.PocketPairUnderBoard || features.OnePairBelowTopBoard)) ||
		(features.FourToStraightBoard && features.Category < poker.Straight)
}
...
func choosePostflopAdjusted(req Request, decision Decision, level int, profile personality.Profile, read OpponentRead) (Action, float64, string, []string, bool) {
	...
	if req.ToCall == 0 {
		...
		fragilePair := decision.Features.Category == poker.OnePair && (decision.Features.OnePairBelowTopBoard || decision.Features.PocketPairUnderBoard)
		switch {
		case decision.Features.FourToStraightBoard && decision.Features.Category < poker.Straight:
			return Check, 0, "POSTFLOP_FOUR_STRAIGHT_POT_CONTROL", []string{"four_to_straight_board", "range_disadvantage", "pot_control"}, false
		case fragilePair:
			return Check, 0, "POSTFLOP_CAPPED_PAIR_POT_CONTROL", []string{"capped_pair", "pot_control"}, false
		case decision.Features.ThreeToFlushBoard && decision.Features.Category == poker.OnePair:
			return Check, 0, "POSTFLOP_THREE_FLUSH_PAIR_POT_CONTROL", []string{"three_to_flush_board", "one_pair", "pot_control"}, false
		case decision.Features.MadeCategoryFromBoard || (decision.Features.PairedBoardTwoPair && req.HeroPostflopAggro > 0):
			return Check, 0, "POSTFLOP_PAIRED_BOARD_POT_CONTROL", []string{"paired_board_made_hand", "kicker_sensitive", "pot_control"}, false
		case decision.Features.PairedBoardTwoPair && eq >= .65+profile.ValueThresholdDelta+read.ValueThreshold:
			return Bet, postflopAmount(req, .5*betMultiplier, false), "POSTFLOP_PAIRED_BOARD_THIN_VALUE", []string{"paired_board_made_hand", "thin_value", "bounded_sizing"}, true
		case decision.Features.PairedBoardTwoPair:
			return Check, 0, "POSTFLOP_PAIRED_BOARD_POT_CONTROL", []string{"paired_board_made_hand", "below_thin_value", "pot_control"}, false
		case decision.Features.Category == poker.OnePair && eq >= .70+profile.ValueThresholdDelta+read.ValueThreshold:
			return Bet, postflopAmount(req, .5*betMultiplier, false), "POSTFLOP_ONE_PAIR_VALUE_BET", []string{"one_pair", "value", "bounded_sizing"}, true
	...
	fourStraightBluffCatch := decision.Features.FourToStraightBoard && decision.Features.Category < poker.Straight
	if fourStraightBluffCatch && (req.RaisesFaced > 0 || req.ActiveOpponents > 1 || req.ToCall >= .5*req.Pot) {
		return Fold, 0, "POSTFLOP_FOUR_STRAIGHT_FOLD", []string{"four_to_straight_board", "range_disadvantage", "bluff_catch_guard"}, false
	}
	pairedBoardBluffCatch := decision.Features.PairedBoardTwoPair || decision.Features.MadeCategoryFromBoard
	if pairedBoardBluffCatch && req.RaisesFaced > 0 {
		return Fold, 0, "POSTFLOP_PAIRED_BOARD_RAISE_FOLD", []string{"paired_board_made_hand", "kicker_sensitive", "raise_guard"}, false
	}
	if underpairBluffCatch && (req.RaisesFaced > 0 || req.ActiveOpponents > 1) {
		return Fold, 0, "POSTFLOP_UNDERPAIR_PRESSURE_FOLD", []string{"pocket_pair_under_board", "range_pressure", "bluff_catch_guard"}, false
	}
	threeFlushPairBluffCatch := decision.Features.ThreeToFlushBoard && decision.Features.Category == poker.OnePair
	if threeFlushPairBluffCatch && (req.RaisesFaced > 0 || req.ActiveOpponents > 1 || req.ToCall >= .5*req.Pot) {
		return Fold, 0, "POSTFLOP_THREE_FLUSH_PAIR_FOLD", []string{"three_to_flush_board", "one_pair", "range_disadvantage", "bluff_catch_guard"}, false
	}
	if req.Street == River && decision.Features.Category == poker.OnePair && req.RaisesFaced > 0 {
		return Fold, 0, "RIVER_ONE_PAIR_RERAISE_FOLD", []string{"one_pair", "river", "opponent_raise", "bluff_catch_guard"}, false
	}
	cappedPairBluffCatch := class == ClassMade && decision.Features.OnePairBelowTopBoard
	if cappedPairBluffCatch && req.RaisesFaced >= 2 {
		return Fold, 0, "POSTFLOP_SECOND_PAIR_RERAISE_FOLD", []string{"one_pair_below_top_board", "repeated_raise", "range_guard"}, false
	}
	if cappedPairBluffCatch && (req.RaisesFaced > 0 || req.ActiveOpponents > 1 || req.ToCall >= .5*req.Pot) {
		return Fold, 0, "POSTFLOP_CAPPED_PAIR_FOLD", []string{"one_pair_below_top_board", "range_disadvantage", "bluff_catch_guard"}, false
	}
	if eq < required {
		rule := "POSTFLOP_FOLD"
	...
	riverOnePair := req.Street == River && decision.Features.Category == poker.OnePair
	if canRaiseForValue && !riverOnePair && !threeFlushPairBluffCatch && !underpairBluffCatch && !pairedBoardBluffCatch && !decision.Features.OnePairBelowTopBoard && (eq >= .74+profile.AggressionDelta || class == ClassMadeStrong && eq >= .50 && eq-decision.PotOdds >= .18+profile.AggressionDelta) {
		if decision.SPR <= 1.2 {
