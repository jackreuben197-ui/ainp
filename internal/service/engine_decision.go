...
import (
	...
	"gitlab.com/smoothsics/ainp/internal/performance"
...
func NewEngineDecisionProvider(cfg config.Config, logger *slog.Logger) *EngineDecisionProvider {
	...
	performanceTracker := performance.NewTracker(cfg.Engine.ProfitControl.TTL, cfg.Engine.ProfitControl.MaxPlayers)
	return newEngineDecisionProvider(cfg, cfg.Engine, tracker, performanceTracker, logger)
}

func newEngineDecisionProvider(cfg config.Config, engineCfg config.EngineConfig, tracker *opponent.Tracker, performanceTracker *performance.Tracker, logger *slog.Logger) *EngineDecisionProvider {
	...
	return &EngineDecisionProvider{
		...
		tracker: tracker, performance: performanceTracker,
...
func (p *EngineDecisionProvider) Decide(ctx context.Context, input DecisionInput) (*protocol.AdviseResponse, error) {
	...
	advice, collapsed, originalAction, originalAmount := finalizeAdviceWithCollapse(
		decisionToAdvice(decision),
		input.State,
		request.NeverFold,
		p.cfg.Strategy.CollapseNearAllIn,
		p.cfg.Strategy.NearAllInRemainingChips,
	)
	if collapsed && p.logger != nil {
		allInAmount := finalizedAdviceValue(advice)
		p.logger.Info("near_allin_collapsed",
			"request_id", input.RequestID,
			"decision_id", input.DecisionID,
			"player_id", input.State.HeroID,
			"table_id", input.State.TableID,
			"hand_id", input.State.HandID,
			"street", input.State.Street,
			"original_action", originalAction,
			"original_amount", originalAmount,
			"allin_amount", allInAmount,
			"remaining_percent", (allInAmount-originalAmount)*100/allInAmount,
		)
	}
	return advice, nil
}

func (p *EngineDecisionProvider) observePerformance(input DecisionInput) {
	if !p.cfg.ProfitControl.Enabled || p.performance == nil || input.State == nil {
		return
	}
	state := input.State
	...
	key := performance.Key{TableID: state.TableID, PlayerID: state.HeroID}
	p.performance.ObserveStart(key, hero.InitialStack)
	profile := p.resolveBotProfile(state.AIProfile)
	if state.LastObservation != nil && state.LastObservation.PlayerID == state.HeroID && profile.BehaviorMode != "aggressive_never_fold" {
		action := string(state.LastObservation.Action)
		if state.LastObservation.CallEquivalent {
			action = string(protocol.ActionCall)
		}
		p.performance.RecordAction(key, state.LastObservation.ObservationID, action)
	}
	if input.Event.Cmd != protocol.CommandEndHand {
		return
	}
	...
	for _, result := range payload.Players {
		if result.PlayerID == state.HeroID {
			p.performance.RecordResult(key, state.HandID, result.Profit)
			return
		}
	}
}

// finalizeAdvice is the last, profile-independent action boundary. Keep the
// dust-remainder collapse last because legal-action and never-fold fallbacks
// can both replace the strategy action with a call.
func finalizeAdvice(advice *protocol.AdviseResponse, state *game.State, neverFold, collapseNearAllIn bool, threshold float64) *protocol.AdviseResponse {
	advice, _, _, _ = finalizeAdviceWithCollapse(advice, state, neverFold, collapseNearAllIn, threshold)
	return advice
}

func finalizeAdviceWithCollapse(advice *protocol.AdviseResponse, state *game.State, neverFold, collapseNearAllIn bool, threshold float64) (*protocol.AdviseResponse, bool, protocol.ActionType, float64) {
	advice = constrainToLegalActions(advice, state)
	if neverFold && advice != nil && advice.Type == protocol.ActionFold {
		advice = constrainNeverFoldToLegalActions(state)
	}
	originalAction := protocol.ActionType("")
	originalAmount := 0.0
	if advice != nil {
		originalAction = advice.Type
		originalAmount = finalizedAdviceValue(advice)
	}
	collapsedAdvice := collapseConstrainedNearAllIn(advice, state, collapseNearAllIn, threshold)
	collapsed := collapsedAdvice != advice && collapsedAdvice != nil && collapsedAdvice.Type == protocol.ActionAllIn
	return collapsedAdvice, collapsed, originalAction, originalAmount
}

func finalizedAdviceValue(advice *protocol.AdviseResponse) float64 {
...
// clamp and every profile-specific fallback. A Call/Bet/Raise of e.g. 119.99
// while All-in is 120 must not leave a visibly artificial 0.01 chip tail.
func collapseConstrainedNearAllIn(advice *protocol.AdviseResponse, state *game.State, enabled bool, threshold float64) *protocol.AdviseResponse {
	if !enabled || threshold <= 0 || advice == nil || advice.Value == nil || state == nil ||
		(advice.Type != protocol.ActionCall && advice.Type != protocol.ActionRaise && advice.Type != protocol.ActionBet) {
		return advice
	}
	...
	if allInAmount <= 0 || remaining <= 0 {
		return advice
	}
	percentThreshold := state.NearAllInRaisePercent
	if advice.Type == protocol.ActionCall {
		percentThreshold = state.NearAllInCallPercent
	}
	if percentThreshold > 0 {
		if remaining*100/allInAmount > percentThreshold+chipEpsilon {
			return advice
		}
	} else if remaining > threshold+chipEpsilon {
		return advice
	}
...
func (p *EngineDecisionProvider) strategyRequest(input DecisionInput) (strategy.Request, error) {
	...
	if p.cfg.ProfitControl.Enabled && p.performance != nil {
		performanceSnapshot = p.performance.Snapshot(performance.Key{TableID: state.TableID, PlayerID: state.HeroID})
	}
	return strategy.Request{
		...
		PostflopSizings: append([]float64(nil), profile.PostflopSizings...),
		...
		HeroPostflopAggro:  hero.PostflopAggro,
		...
		PreflopOpenCallGap: p.cfg.Strategy.PreflopOpenCallGap, PreflopReraiseEquity: p.cfg.Strategy.PreflopReraiseEquity, PreflopReraiseRangeFactor: p.cfg.Strategy.PreflopReraiseRangeFactor,
		...
		PreflopCallMargin: p.cfg.Strategy.PreflopCallMargin, PreflopLargeCallBB: p.cfg.Strategy.PreflopLargeCallBB,
		...
		UnderpairCallMargin:    p.cfg.Strategy.UnderpairCallMargin,
		TurnWeakDrawCallMargin: p.cfg.Strategy.TurnWeakDrawCallMargin, RiverBoardPairCallMargin: p.cfg.Strategy.RiverBoardPairCallMargin,
		RiverMissedDrawMargin: p.cfg.Strategy.RiverMissedDrawMargin,
		RejectNegativeEVCalls: p.cfg.Strategy.RejectNegativeEVCalls,
		TablePerformance: strategy.TablePerformance{
			InitialStack: performanceSnapshot.InitialStack, NetProfit: performanceSnapshot.NetProfit,
			Hands: performanceSnapshot.Hands, Actions: performanceSnapshot.Actions,
			Calls: performanceSnapshot.Calls, Aggressive: performanceSnapshot.Aggressive,
		},
		ProfitControl: strategy.ProfitControl{
			Enabled: p.cfg.ProfitControl.Enabled, MinimumActions: p.cfg.ProfitControl.MinimumActions,
			LargeActionBB: p.cfg.ProfitControl.LargeActionBB, LargeActionStackRatio: p.cfg.ProfitControl.LargeActionStackRatio,
			ProfitTriggerBB: p.cfg.ProfitControl.ProfitTriggerBB, ProfitTriggerStackRatio: p.cfg.ProfitControl.ProfitTriggerStackRatio,
			LossTriggerBB: p.cfg.ProfitControl.LossTriggerBB, LossTriggerStackRatio: p.cfg.ProfitControl.LossTriggerStackRatio,
			CallRateTarget: p.cfg.ProfitControl.CallRateTarget, AggressionRateTarget: p.cfg.ProfitControl.AggressionRateTarget,
			MaxExposureMargin: p.cfg.ProfitControl.MaxExposureMargin, MaxPerformanceMargin: p.cfg.ProfitControl.MaxPerformanceMargin,
			MaxActionMixMargin: p.cfg.ProfitControl.MaxActionMixMargin, MaxTotalMargin: p.cfg.ProfitControl.MaxTotalMargin,
		},
	}, nil
}
...
func (p *EngineDecisionProvider) resolveBotProfile(aiProfile string) botProfileResolution {
	...
	if p.cfg.Personality.UseAIProfile {
		if configured, ok := p.cfg.Personality.Profiles[profileKey]; ok {
			...
			result.PostflopSizings = append([]float64(nil), configured.PostflopSizings...)
