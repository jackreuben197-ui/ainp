...
func TestProviderTracksRawTableProfitAndNormalActionMix(t *testing.T) {
	cfg := config.Default()
	cfg.Engine.Personality.Profiles["FPCH_100_50"] = config.BotProfileConfig{
		Personality: "lag", Level: 5, TargetVPIP: 1, TargetPFR: .5,
		BehaviorMode: "aggressive_never_fold", NeverFold: true,
	}
	provider := NewEngineDecisionProvider(cfg, slog.New(slog.NewTextHandler(io.Discard, nil)))
	state := &game.State{
		HeroID: "bot", TableID: "table", HandID: "hand-1", AIProfile: "FPCH_default",
		Players: map[string]*game.Player{"bot": {ID: "bot", InitialStack: 200, Stack: 180}},
	}
	provider.observePerformance(DecisionInput{State: state})
	state.LastObservation = &game.Observation{ObservationID: "action-1", PlayerID: "bot", Action: protocol.ActionCall}
	provider.observePerformance(DecisionInput{State: state})
	provider.observePerformance(DecisionInput{State: state})
	payload, err := json.Marshal(protocol.EndHandPayload{Players: []protocol.PlayerEndHand{{PlayerID: "bot", Profit: -37.5}}})
	if err != nil {
		t.Fatal(err)
	}
	provider.observePerformance(DecisionInput{State: state, Event: protocol.EventRequest{Cmd: protocol.CommandEndHand, Payload: payload}})
	provider.observePerformance(DecisionInput{State: state, Event: protocol.EventRequest{Cmd: protocol.CommandEndHand, Payload: payload}})

	got := provider.performance.Snapshot(performance.Key{TableID: "table", PlayerID: "bot"})
	if got.InitialStack != 200 || got.NetProfit != -37.5 || got.Hands != 1 || got.Actions != 1 || got.Calls != 1 {
		t.Fatalf("snapshot=%+v", got)
	}

	state.AIProfile = "FPCH_100_50"
	state.LastObservation = &game.Observation{ObservationID: "action-2", PlayerID: "bot", Action: protocol.ActionAllIn}
	provider.observePerformance(DecisionInput{State: state})
	got = provider.performance.Snapshot(performance.Key{TableID: "table", PlayerID: "bot"})
	if got.Actions != 1 || got.Aggressive != 0 {
		t.Fatalf("special action polluted normal mix=%+v", got)
	}
}
