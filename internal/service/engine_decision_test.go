...
func TestResolveBotProfilePerHand(t *testing.T) {
	cfg := config.Default()
	cfg.Engine.Personality.Profiles["AICON_TAG_L2"] = config.BotProfileConfig{Personality: "tag", Level: 2, TargetVPIP: .30, TargetPFR: .15}
	...
	if first.PersonalityID != "tag" || first.Level != 2 || first.TargetVPIP != .30 || first.TargetPFR != .15 || first.Source != "profiles" || second.PersonalityID != "lag" || second.Level != 5 || second.TargetVPIP != .45 || second.TargetPFR != .25 || second.Source != "profiles" {
...
func TestStrategyRequestUsesCurrentHandsAIProfile(t *testing.T) {
	cfg := config.Default()
	cfg.Engine.Personality.Profiles["TAG_L2"] = config.BotProfileConfig{Personality: "tag", Level: 2, TargetVPIP: .30, TargetPFR: .15}
	...
	if request.AIProfile != "TAG_L2" || request.PersonalityID != "tag" || request.Level != 2 || request.TargetVPIP != .30 || request.TargetPFR != .15 {
...
func TestCollapseConstrainedNearAllInUsesServerAllInAmount(t *testing.T) {
	...
	for _, actionType := range []protocol.ActionType{protocol.ActionBet, protocol.ActionRaise} {
		value := 119.99
