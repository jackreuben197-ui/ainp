...
func TestDefaultEngineConfigurationIsValid(t *testing.T) {
	...
	if !cfg.Engine.ProfitControl.Enabled || cfg.Engine.ProfitControl.TTL <= 0 || cfg.Engine.ProfitControl.LargeActionBB <= 0 || cfg.Engine.ProfitControl.MaxTotalMargin <= 0 {
		t.Fatalf("profit control defaults=%+v", cfg.Engine.ProfitControl)
	}
}

func TestProfitControlCanBeDisabledWithoutTuningValues(t *testing.T) {
	cfg := Default()
	cfg.Auth.Token = "test"
	cfg.Engine.ProfitControl = ProfitControlConfig{}
	if err := cfg.Validate(); err != nil {
		t.Fatalf("disabled profit control: %v", err)
	}
	cfg.Engine.ProfitControl.Enabled = true
	if err := cfg.Validate(); err == nil {
		t.Fatal("enabled profit control must reject empty tuning")
	}
}
