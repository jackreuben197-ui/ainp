...
func TestMemoryHandStoreSequenceAndIdempotency(t *testing.T) {
	...
	key := HandKey{PlayerID: "bot", RoomID: "np", TableID: "1", HandID: "2"}
	now := time.Now()
...
func TestMemoryHandStoreCapacityEvictsOldest(t *testing.T) {
	...
	for index, handID := range []string{"one", "two", "three"} {
		key := HandKey{PlayerID: "bot", RoomID: "np", TableID: "1", HandID: handID}
	...
	oldest := HandKey{PlayerID: "bot", RoomID: "np", TableID: "1", HandID: "one"}
