...
func (s *State) applyAction(req protocol.EventRequest) error {
	...
	if payload.ValueMode == "street_total" && payload.Type != protocol.ActionFold && payload.Type != protocol.ActionCheck {
		rawValue := value
	...
	if payload.PlayerID == s.HeroID && s.PendingAdvice != nil {
		comparison := payload
		if payload.Type == protocol.ActionFold || payload.Type == protocol.ActionCheck {
