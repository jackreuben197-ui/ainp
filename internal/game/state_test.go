...
func TestShortStraddleKeepsFullTwoBigBlindBringIn(t *testing.T) {
	state, err := New(gameEvent(1, "btn", protocol.CommandStartHand, map[string]any{
		"game_type": "NLH", "club_id": "1", "ai_profile": "tag", "time": 1, "small_blind": 10, "big_blind": 20, "time_to_act": 12000, "max_seat": 4,
		"players": []map[string]any{
			{"player_id": "sb", "nick": "sb", "stack": 100, "role": "sb"},
			{"player_id": "bb", "nick": "bb", "stack": 100, "role": "bb"},
			{"player_id": "st", "nick": "st", "stack": 45, "role": "st"},
			{"player_id": "btn", "nick": "btn", "stack": 100, "role": "bt"},
		},
		"antes":    []map[string]any{{"player_id": "sb", "value": 10}, {"player_id": "bb", "value": 10}, {"player_id": "st", "value": 10}, {"player_id": "btn", "value": 10}},
		"blinds":   []map[string]any{{"player_id": "sb", "value": 10, "type": "small_blind"}, {"player_id": "bb", "value": 20, "type": "big_blind"}, {"player_id": "st", "value": 35, "type": "straddle"}},
		"straddle": 35,
	}))
	if err != nil {
		t.Fatal(err)
	}
	if state.CurrentBet != 40 || !state.Players["st"].AllIn {
		t.Fatalf("current_bet=%v straddle=%+v", state.CurrentBet, state.Players["st"])
	}
	if err := state.Apply(gameEvent(2, "btn", protocol.CommandAction, map[string]any{
		"player_id": "btn", "type": "call", "value": 40, "value_mode": "increment", "stack_after": 50, "next_player_id": "bb",
	})); err != nil {
		t.Fatal(err)
	}
	if err := state.Apply(gameEvent(3, "btn", protocol.CommandAction, map[string]any{
		"player_id": "bb", "type": "call", "value": 20, "value_mode": "increment", "stack_after": 50, "next_player_id": "",
	})); err != nil {
		t.Fatal(err)
	}
	if state.Players["btn"].StreetContribution != 40 || state.Players["bb"].StreetContribution != 40 {
		t.Fatalf("button=%+v big_blind=%+v", state.Players["btn"], state.Players["bb"])
	}
}

func TestNonAllInOneBigBlindStraddleKeepsReportedBringIn(t *testing.T) {
	state, err := New(gameEvent(1, "btn", protocol.CommandStartHand, map[string]any{
		"game_type": "NLH", "club_id": "1", "ai_profile": "tag", "time": 1, "small_blind": 10, "big_blind": 20, "time_to_act": 12000, "max_seat": 4,
		"players": []map[string]any{
			{"player_id": "sb", "nick": "sb", "stack": 100, "role": "sb"},
			{"player_id": "bb", "nick": "bb", "stack": 100, "role": "bb"},
			{"player_id": "st", "nick": "st", "stack": 200, "role": "st"},
			{"player_id": "btn", "nick": "btn", "stack": 100, "role": "bt"},
		},
		"antes":    []map[string]any{{"player_id": "sb", "value": 10}, {"player_id": "bb", "value": 10}, {"player_id": "st", "value": 10}, {"player_id": "btn", "value": 10}},
		"blinds":   []map[string]any{{"player_id": "sb", "value": 10, "type": "small_blind"}, {"player_id": "bb", "value": 20, "type": "big_blind"}, {"player_id": "st", "value": 20, "type": "straddle"}},
		"straddle": 20,
	}))
	if err != nil {
		t.Fatal(err)
	...
	if state.CurrentBet != 20 {
		t.Fatalf("current_bet=%v want reported 20", state.CurrentBet)
	}
	if err := state.Apply(gameEvent(2, "btn", protocol.CommandAction, map[string]any{
		"player_id": "btn", "type": "check", "value": 0, "value_mode": "increment", "stack_after": 70, "next_player_id": "sb",
	})); err != nil {
		t.Fatal(err)
	}
}

func TestFoldedHeroToleratesIncompleteSpectatorActionStream(t *testing.T) {
	state, err := New(gameEvent(1, "hero", protocol.CommandStartHand, map[string]any{
		"game_type": "NLH", "club_id": "1", "ai_profile": "tag", "time": 1, "small_blind": 10, "big_blind": 20, "time_to_act": 12000, "max_seat": 3,
		"players": []map[string]any{{"player_id": "sb", "nick": "sb", "stack": 100, "role": "sb"}, {"player_id": "bb", "nick": "bb", "stack": 100, "role": "bb"}, {"player_id": "hero", "nick": "hero", "stack": 100}},
		"blinds":  []map[string]any{{"player_id": "sb", "value": 10, "type": "small_blind"}, {"player_id": "bb", "value": 20, "type": "big_blind"}},
	}))
	if err != nil {
		t.Fatal(err)
	...
	if err := state.Apply(gameEvent(2, "hero", protocol.CommandAction, map[string]any{
		"player_id": "hero", "type": "fold", "value": 0, "value_mode": "increment", "stack_after": 80, "next_player_id": "sb",
	})); err != nil {
		t.Fatal(err)
	}
	state.CurrentBet = 50
	// Simulate a missed BB call and street transition: the next observed check
	// is invalid against the local bet, but the folded hero can never act again.
	if err := state.Apply(gameEvent(3, "hero", protocol.CommandAction, map[string]any{
		"player_id": "bb", "type": "check", "value": 0, "value_mode": "increment", "stack_after": 80, "next_player_id": "sb",
	})); err != nil {
		t.Fatal(err)
	}
	if state.LastNormalization != "folded_hero_stale_action" || state.LastObservation != nil {
		t.Fatalf("normalization=%q observation=%+v", state.LastNormalization, state.LastObservation)
	}
}
