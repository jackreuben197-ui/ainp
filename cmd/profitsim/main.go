...
func main() {
	...
	disableProfitControl := flag.Bool("disable-profit-control", false, "disable only the table profit/action-mix control")
	...
	if *disableRisk {
		...
		cfg.Engine.ProfitControl.Enabled = false
	}
	if *disableProfitControl {
		cfg.Engine.ProfitControl.Enabled = false
	}
	...
	result, err := simulateWithProfitControl(*profileName, profile, profitControl(cfg.Engine.ProfitControl), *hands, *samples, *rake)
	if err != nil {
		fatal(err)
	}
...
func simulate(name string, profile config.BotProfileConfig, hands, samples int, rake float64) (report, error) {
	return simulateWithProfitControl(name, profile, strategy.ProfitControl{}, hands, samples, rake)
}

func simulateWithProfitControl(name string, profile config.BotProfileConfig, control strategy.ProfitControl, hands, samples int, rake float64) (report, error) {
	started := time.Now()
	result := report{GeneratedAt: started, Profile: name, Opponent: "heads-up loose-passive benchmark v1", Hands: hands, StartingStackBB: 100, RakePercent: rake, ProfitControlEnabled: control.Enabled}
	engine := strategy.NewEngine(equity.NewCalculator())
	ledger := &simulationLedger{TablePerformance: strategy.TablePerformance{InitialStack: 100}}
	sumSquares := 0.0
	for hand := 0; hand < hands; hand++ {
		cards := deal(uint64(hand + 1))
		outcome, err := playHand(engine, profile, control, ledger, cards, int64(hand+1), samples, rake)
		if err != nil {
			return result, fmt.Errorf("hand %d: %w", hand, err)
		}
		result.NetProfitBB += outcome.profit
		ledger.NetProfit += outcome.profit
		ledger.Hands++
	...
	result.ProfitControlApplications = ledger.applications
	return result, nil
}

func playHand(engine *strategy.Engine, profile config.BotProfileConfig, control strategy.ProfitControl, ledger *simulationLedger, dealt [9]poker.Card, seed int64, samples int, rake float64) (handResult, error) {
	hero, villain, board := dealt[:2], dealt[2:4], dealt[4:]
	heroInvested, villainInvested, pot := .5, 1.0, 1.5
	heroStack, villainStack := 99.5, 99.0
	preflop, err := engine.Decide(context.Background(), request(profile, control, ledger.TablePerformance, strategy.Preflop, hero, nil, pot, .5, heroStack, seed, samples))
	if err != nil {
		return handResult{}, err
	}
	ledger.record(preflop)
	...
	for index, street := range []strategy.Street{strategy.Flop, strategy.Turn, strategy.River} {
		...
		decision, decideErr := engine.Decide(context.Background(), request(profile, control, ledger.TablePerformance, street, hero, visible, pot-villainBet, villainBet, heroStack, seed+int64(index+1)*101, samples))
		if decideErr != nil {
...
func request(profile config.BotProfileConfig, control strategy.ProfitControl, performance strategy.TablePerformance, street strategy.Street, hero, board []poker.Card, pot, toCall, stack float64, seed int64, samples int) strategy.Request {
	...
	return strategy.Request{Game: equity.GameNLH, Street: street, Position: strategy.BTN, Hero: hero, Board: board, Opponents: [][]poker.Card{{}}, Pot: pot, ToCall: min(toCall, stack), Stack: stack, EffectiveStack: stack, BigBlind: 1, Level: profile.Level, TargetVPIP: profile.TargetVPIP, TargetPFR: profile.TargetPFR, PersonalityID: profile.Personality, BehaviorMode: profile.BehaviorMode, NeverFold: profile.NeverFold, PostflopCallMargin: profile.PostflopCallMargin, LargePotThresholdBB: profile.LargePotThreshold, LargePotMinEquity: profile.LargePotMinEquity, DisableOpponentModel: true, DisableThinkTime: true, Seed: seed, EquitySamples: samples, MaxExactOutcomes: 1, LegalActions: actions, ProfitControl: control, TablePerformance: performance}
}

func (l *simulationLedger) record(decision strategy.Decision) {
	l.Actions++
	switch decision.Action {
	case strategy.Call:
		l.Calls++
	case strategy.Bet, strategy.Raise, strategy.AllIn:
		l.Aggressive++
	}
	if decision.ProfitControl.Applied {
		l.applications++
	}
}

func profitControl(cfg config.ProfitControlConfig) strategy.ProfitControl {
	return strategy.ProfitControl{
		Enabled: cfg.Enabled, MinimumActions: cfg.MinimumActions,
		LargeActionBB: cfg.LargeActionBB, LargeActionStackRatio: cfg.LargeActionStackRatio,
		ProfitTriggerBB: cfg.ProfitTriggerBB, ProfitTriggerStackRatio: cfg.ProfitTriggerStackRatio,
		LossTriggerBB: cfg.LossTriggerBB, LossTriggerStackRatio: cfg.LossTriggerStackRatio,
		CallRateTarget: cfg.CallRateTarget, AggressionRateTarget: cfg.AggressionRateTarget,
		MaxExposureMargin: cfg.MaxExposureMargin, MaxPerformanceMargin: cfg.MaxPerformanceMargin,
		MaxActionMixMargin: cfg.MaxActionMixMargin, MaxTotalMargin: cfg.MaxTotalMargin,
	}
}
