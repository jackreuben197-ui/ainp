...
func TestStateOnlyReplayRenumbersEventsAfterHistoricallyRejectedInput(t *testing.T) {
	...
	records := []replay.Record{
		{Fingerprint: "start", Outcome: "applied", Event: protocol.EventRequest{SeqNum: 1, PlayerID: "bot", RoomID: "np", TableID: "table", HandID: &handID, Cmd: protocol.CommandStartHand, Payload: []byte(`{"game_type":"NLH","club_id":"1","ai_profile":"FPCH_default","time":1,"small_blind":1,"big_blind":2,"time_to_act":12000,"max_seat":2,"players":[{"player_id":"bot","nick":"bot","stack":100},{"player_id":"villain","nick":"villain","stack":100}],"blinds":[]}`)}},
		// The old build rejected this valid zero-value fold, so the historical
		// adapter reused seq_num=2 for the next distinct event.
		{Fingerprint: "fold", Outcome: "rejected", Event: protocol.EventRequest{SeqNum: 2, PlayerID: "bot", RoomID: "np", TableID: "table", HandID: &handID, Cmd: protocol.CommandAction, Payload: []byte(`{"player_id":"villain","type":"fold","value":0,"value_mode":"street_total","stack_after":100,"next_player_id":"bot"}`)}},
		{Fingerprint: "deal", Outcome: "applied", Event: protocol.EventRequest{SeqNum: 2, PlayerID: "bot", RoomID: "np", TableID: "table", HandID: &handID, Cmd: protocol.CommandDealCards, Payload: []byte(`{"cards":"AsKd","next_player_id":"bot"}`)}},
	}
...
func TestReplaySkipsOnlyTruncatedFinalRecord(t *testing.T) {
	...
	record := replay.Record{Fingerprint: "start", Outcome: "applied", Event: protocol.EventRequest{
		SeqNum: 1, PlayerID: "bot", RoomID: "np", TableID: "table", HandID: &handID,
